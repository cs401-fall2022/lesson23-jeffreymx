...
router.get('/', function (req, res, next) {
  var db = new sqlite3.Database('jsmBlogs.sqlite3',
        ...
        (err, rows) => {
          if (rows.length === 1) {
            ...
            db.all(` select blog_id, blog_type, blog_txt from blog`, (err, rows) => {
              ...
              res.render('index', { title: 'Jeff\'s Experimental Page', data: rows });
            });
          } else {
            ...
            db.exec(`create table blog (
                     ...
                     blog_txt text NOT NULL);
                      ...
                      values ('math','You should check out these cool videos.  I especially like video 5.  www.youtube.com/playlist?list=PL97CCC2CC4E89C7E5'),
  ...
  var db = new sqlite3.Database('jsmBlogs.sqlite3',
    ...
    (err) => {
      ...
      db.exec(`insert into blog (blog_type, blog_txt)
                values ('${req.body.topic}','${req.body.newPost}');`)
